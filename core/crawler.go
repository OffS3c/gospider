...
import (
	...
	"github.com/OffS3c/gospider/stringset"
...
func NewCrawler(site *url.URL, cmd *cobra.Command) *Crawler {
	...
	// # disallowedRegex := `(?i)\.(png|apng|bmp|gif|ico|cur|jpg|jpeg|jfif|pjp|pjpeg|svg|tif|tiff|webp|xbm|3gp|aac|flac|mpg|mpeg|mp3|mp4|m4a|m4v|m4p|oga|ogg|ogv|mov|wav|webm|eot|woff|woff2|ttf|otf|css)(?:\?|#|$)` # This is the original code
...
func (crawler *Crawler) Start(linkfinder bool) {
	...
	crawler.C.OnHTML("[href]", func(e *colly.HTMLElement) {
		...
		if !crawler.urlSet.Duplicate(urlString) {
			...
			// _ = e.Request.Visit(urlString) # This is the original code and 4 lines above is the new code
		}
	})
